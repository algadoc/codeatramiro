...
function handleCoursesButton(event){
  if(!isanimationrunning){ //Executes if the animation is not running, improves stability
    ...
    if (iscoursesactive){ //If the courses div should show
      animateElement("logodiv","fadeoutdown","grid","none",1000); //Fades out the logodiv
      animateElement("contentdiv","fadeoutdown","flex","none",1000); //Fades out the content
      ...
      setTimeout(() =>{ //After the other animations are done, start the next one
        ...
        setTimeout(() => {isanimationrunning = false;},2300); //Return the isanimationrunning to false, for a new change
